...
module parameters
  ...
  integer, parameter :: nstep = 10000          ! total no. of timee steps..

  double precision, parameter :: Tb = 1.0d0  ! initial temperature of the system..
  
  double precision, parameter :: dens = 0.40d0     ! number density
  ...
  integer, parameter :: traj_dump = 10  ! dump trajectory 

  !  Calculate the box length 
  double precision, parameter :: vol = npart/dens
  double precision, parameter :: boxl = (vol)**(1.0/3.0)
...
program md_nvt_basics
  ...
  double precision :: t
  double precision :: lambda, virial, pressure
  ...
  do istep = 1, nstep  ! loop over the time...
    ...
    if ((istep .gt. 1000) .and. (mod(istep,traj_dump) .eq. 0)) then
       write(103,*) npart
       write(103,*) 
       do i = 1, npart
...
subroutine setup
  ...
  double precision :: ke, pe, Tinst, virial
